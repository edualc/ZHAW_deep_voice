...
from networks.resnet50.core.data_gen import generate_test_data_h5
# TODO
...
class ActiveLearningUncertaintyCallback(Callback):
    ...
    def _perform_uncertainty_sampling(self, speaker):
        ...
        ys = self.model.predict(Xb.reshape(spectrograms.shape[0], self.segment_size, self.spectrogram_height, 1))
...
class EERCallback(Callback):
    ...
    def _calculate_and_log_eer(self):
        ...
        # 
        start = time.time()
        out_layer = self.config.getint('resnet50', 'out_layer')
        model_partial = Model(
