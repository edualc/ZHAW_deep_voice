...
from .resnet_network import resnet_network
...
class ResNet50Controller(NetworkController):
    def __init__(self, name, config, dev, best):
        super().__init__(name, config, dev)

        self.spectrogram_height = config.getint('resnet50','spectrogram_height')
        self.seg_size = config.getint('resnet50', 'seg_size')
        self.out_layer = config.getint('resnet50', 'out_layer')
        self.vec_size = config.getint('resnet50', 'vec_size')
