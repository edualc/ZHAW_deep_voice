...
def generate_evaluation_data(utterances_list, dataset_path, segment_size, spectrogram_height):
    X_eval = np.zeros((len(utterances_list), segment_size, spectrogram_height, 1))
    ...
    with h5py.File(dataset_path + '.h5', 'r') as f:
        for element in utterances_list:
            ...
            X_eval[index,:,:,0] = spect[seg_idx:seg_idx + segment_size, :]
...
def batch_generator_h5(batch_type, dataset, batch_size=100, segment_size=40, spectrogram_height=128):
    ...
    while 1:
        for i in range((num_segments // batch_size) + 1):
            ...
            Xb = np.zeros((batch_size, segment_size, spectrogram_height, 1), dtype=np.float32)
            yb = np.zeros(batch_size, dtype=np.int32)
